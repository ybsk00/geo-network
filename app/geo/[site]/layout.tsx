...
const NAVER_VERIFICATION: Record<string, string> = {
  [ROOT_DOMAIN]: "2902ed53731cb3fccd6747d0cdc4e308bedd277d",
...
const PORTAL_FALLBACK: SiteConfig = {
  ...
  description:
    "GEO Networks는 의료·건강 정보부터 비즈니스·테크 인사이트까지 30여 개 독립 매체를 운영하는 큐레이션 네트워크입니다.",
  tone: "neutral",
  language: "ko",
  theme: {
    primaryColor: "#1e293b",
...
function resolveSiteFromParam(siteParam: string): SiteConfig {
  if (siteParam === ROOT_SITE_SLUG) return PORTAL_FALLBACK;
  return getSiteById(siteParam) ?? PORTAL_FALLBACK;
}
...
export async function generateMetadata({
  ...
  if (siteParam === ROOT_SITE_SLUG) {
    return {
      title: {
        default: "GEO Networks — 건강·의료·비즈니스 콘텐츠 네트워크",
        template: "%s | GEO Networks",
      },
      description: PORTAL_FALLBACK.description,
  ...
  const naverCode =
    NAVER_VERIFICATION[site.domain] ?? NAVER_VERIFICATION[ROOT_DOMAIN];
...
export default async function SiteLayout({
  ...
  const { site: siteParam } = await params;
  const site = resolveSiteFromParam(siteParam);
  const isPortal = siteParam === ROOT_SITE_SLUG;
  const t = site.theme;
      ...
      >
        ...
        {!isPortal && (
          <footer
            ...
            style={{ borderTopColor: `${t.primaryColor}20` }}
          >
